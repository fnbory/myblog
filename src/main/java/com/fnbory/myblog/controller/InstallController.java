...
import com.fnbory.myblog.common.enums.ThemeStatus;
...
public class InstallController {
	...
	@ResponseBody
	public CommonResult execute(@RequestParam(value = "blogName") String blogName,
		...
		if(!StrUtil.equals(nowPwd, confirmPwd)) {
			return new CommonResult(204,"两次密码不一样，请重新输入！");
		}
		try {
		//创建用户
		User user=new User();
		...
		article.setArticleCommentStatus(ThemeStatus.THEME_NOT_ENABLED.getValue());
...
//				ServletUtil.getClientIP(request), DateUtil.date()));
		...
		theme.setThemeThumbnail("/static/img/pinghsu.jpg");
		theme.setThemeCreateTime(DateUtil.date());
		theme.setThemeStatus(1);
		themeService.saveTheme(theme);
		BlogConst.THEME_NAME="pinghsu";
		} catch (Exception e) {
			//log.error(e.getMessage());
			return new CommonResult(204, "系统错误");
		}	
